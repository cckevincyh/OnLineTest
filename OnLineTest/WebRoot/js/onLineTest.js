$(function() {
			...
			if(!hasSetCookie()){//如果没有这个cookie就新增一个
				addCookie($.trim($("#test_studentId").val()), _expiresHours, _expiresHours);	//cookieName：考生的id,cookieValue:考试时间，expiresHours：cookie过期时间
			} 
			//开始倒计时
		...
		function hasSetCookie(){
			...
			for (var i = 0; i < arrCookie.length; i++) {
				...
				//找到名称为考试id的cookie，则返回true,否则返回false
				if (arr[0] == $.trim($("#test_studentId").val())) {
					return true;
				}
			};
			return false;
		...
		function settime(remainTime) {
			var _countdown = parseInt(getCookieValue($.trim($("#test_studentId").val()))) / 1000;	//得到倒数秒
			...
			if (_countdown <= 0) {	//倒数结束
				...
				_countdown--;
				editCookie($.trim($("#test_studentId").val()), _countdown * 1000, _countdown * 1000);
			}
			//每1000毫秒执行一次
			setTimeout(function() {
				settime(remainTime);
